...
class ZyyQCAWImp(QCAWImp):
    ...
    def upNodeWeight(self, node_str):
        ...
        for nnode_idx in out_nb_set:
            # delt_q = self._delta_q(nnode_idx, node._com, node._nb[nnode_idx])
...
if __name__ == "__main__":
    assert len(sys.argv) == 2, 'Usage:\t%s <link_fname>' % sys.argv[0]
